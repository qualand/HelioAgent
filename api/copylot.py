...
from api.pysoltrace import Point, PySolTrace
import math
...
class CoPylot:
    ...
    def __init__(self, debug: bool = False):
        ...
        if sys.platform == 'win32' or sys.platform == 'cygwin':
            if is_debugging:
                ...
                self.pdll = CDLL(cwd + "/api/solarpilot.dll")
    ...
    def __unitvect(self, pt : Point ):
        r = (pt.x*pt.x + pt.y*pt.y + pt.z*pt.z)**0.5
        return Point( pt.x/r, pt.y/r, pt.z/r )
    ...
    def set_data(self, p_data, D):          # TODO: add this to the SolarPILOT repo
        """Set values in dictionary D in CoPylot instance
        ...
        p_data : int
            memory address of SolarPILOT instance
        D : str
            Dictionary of SolarPILOT variable names and values 

        Returns
        -------
        bool
            Were all variables successfully set
        """
        all_vars_set = True
        for k,val in D.items():
            ok = False
            if type(val) == type('str'):
                ok = self.data_set_string(p_data, k, val)
            elif type(val) == type(1.0) or type(val) == type(1) or type(val) == type(True) or type(val) == type(np.array([1.0])[0]) or type(val) == type(np.array([1])[0]):
                ok = self.data_set_number(p_data, k, val)
            elif type(val) == type([0]):
                if type(val[0]) == type([0]):
                    ok = self.data_set_matrix(p_data, k, val)
                else:
                    ok = self.data_set_array(p_data, k, val)
                
            if not ok:
                print ('Error: Parameter value for %s not set'%k)
            all_vars_set = all_vars_set and ok
        return all_vars_set
    ...
    def load_soltrace_structure(self, p_data: int):
        ...
        **Follows CreateSTSystem in the cpp code / STObject.cpp**
            ...
            for j in range(ncantx):
                for k in range(ncanty):
                    ...
                    if isdetail:
                        ...
                        Floc = Point( H[f"{panel_name}_x"], H[f"{panel_name}_y"], H[f"{panel_name}_z"] )
                        Faim = Point( H[f"{panel_name}_i"], H[f"{panel_name}_j"], H[f"{panel_name}_k"] )
                        Faim = self.__unitvect(Faim) 

                        #Rotate to match heliostat rotation
                        Floc = P.util_rotation_arbitrary(track_zen, Point(1,0,0), Point(), Floc)
                        Faim = P.util_rotation_arbitrary(track_zen, Point(1,0,0), Point(), Faim)
                        Floc = P.util_rotation_arbitrary(math.pi - track_az, Point(0,0,1), Point(), Floc)
                        Faim = P.util_rotation_arbitrary(math.pi - track_az, Point(0,0,1), Point(), Faim)
        ...
        if recgeom == 0:  #CYLINDRICAL_CLOSED:
            ...
            aim = Point(math.cos(el)*math.sin(az), math.cos(el)*math.cos(az), math.sin(el))
            ...
            aim = Point(math.sin(el)*math.cos(az), math.sin(el)*math.sin(az), math.cos(el))
            element.aim.x = element.position.x + aim.x*1000.
            element.aim.y = element.position.y + aim.y*1000.
            element.aim.z = element.position.z + aim.z*1000.
